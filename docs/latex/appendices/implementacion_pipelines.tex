\section*{APÉNDICE E: IMPLEMENTACIÓN DETALLADA DE PIPELINES DE EXTRACCIÓN}
\addcontentsline{toc}{section}{Apéndice E: Implementación Detallada de Pipelines}
...
\subsubsection*{E.1.1. Justificación del Enfoque Dual NER + Regex}
...
\subsubsection*{E.1.2. Componentes del Pipeline A}
...
\begin{itemize}
    \item \textbf{Patrones}: 371 expresiones regulares compiladas organizadas en 18 categorías:
        \begin{itemize}
            ...
            \item Skills técnicas O*NET + manuales (235): Taxonomías externas (152 O*NET + 83 manuales) para ampliar cobertura
...
\begin{itemize}
    ...
    \item \textbf{Normalización}: Diccionario canónico de 200+ equivalencias (``js'' a ``JavaScript'', ``k8s'' a ``Kubernetes'')
...
\subsubsection*{E.1.3. Flujo de Integración y Procesamiento}
...
\begin{enumerate}
    ...
    \item \textbf{Normalización canónica}: Se aplica diccionario de equivalencias
        \begin{itemize}
            ...
            \item Ejemplos: ``js'' a ``JavaScript'', ``k8s'' a ``Kubernetes'', ``postgres'' a ``PostgreSQL''
...
\subsubsection*{E.1.4. Control de Calidad y Validación}
...
\subsubsection*{E.1.5. Evaluación Detallada del Pipeline A}
...
\subsection*{E.2. Pipeline B: Implementación con Large Language Models}
...
\subsubsection*{E.2.1. Justificación del Enfoque LLM}
...
\subsubsection*{E.2.2. Selección del Modelo}
...
\subsubsection*{E.2.3. Arquitectura de Prompt Engineering}
...
\begin{itemize}
    ...
    \item \textbf{Normalización inline}: Especificar transformaciones como ``postgres a PostgreSQL, k8s a Kubernetes'' redujo variantes léxicas de 18\% a 4\%
    \item \textbf{Formato JSON estricto}: Exigir ``ÚNICAMENTE el objeto JSON'' permitió parsing automático con 99\% éxito
\end{itemize}

\subsubsection*{E.2.4. Configuración de Inferencia}
...
\subsubsection*{E.2.5. Evaluación Detallada del Pipeline B}
...
\subsection*{E.3. Conclusiones de Implementación}
