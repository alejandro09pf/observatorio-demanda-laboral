...
Este trabajo diseñó, implementó y validó un sistema completo de observatorio de demanda laboral para América Latina, comparando tres enfoques de extracción de habilidades técnicas: métodos basados en reglas y reconocimiento de entidades nombradas (Pipeline A), métodos estadísticos con TF-IDF y n-gramas (Pipeline A.1), y modelos de lenguaje grandes (Pipeline B). La evaluación rigurosa sobre un \textit{gold standard} de 7,848 anotaciones manuales demostró la superioridad de Pipeline B, validando las hipótesis de investigación planteadas y estableciendo métricas cuantitativas para la comparación de enfoques de extracción de habilidades en ofertas laborales.

\section{Validación de Hipótesis}

\subsection{H1: Superioridad de Modelos de Lenguaje Grandes}

La primera hipótesis planteaba que los modelos de lenguaje grandes (LLMs) superarían a métodos tradicionales basados en reglas y reconocimiento de entidades nombradas. Los resultados experimentales presentados en el Capítulo 7 validan esta hipótesis de manera concluyente:

\begin{itemize}
    \item Pipeline B alcanzó un F1-Score post-ESCO de \textbf{84.26\%}, superando en 12.11 puntos porcentuales a Pipeline A (72.15\%)
    \item La mejora relativa del \textbf{16.8\%} en F1-Score demuestra una ventaja sustancial
    \item En términos de precisión, Pipeline B obtuvo \textbf{89.25\%} vs 66.28\% de Pipeline A, una mejora relativa del 34.6\%
    \item Incluso en evaluación pre-ESCO (extracción pura), Pipeline B alcanzó F1=46.23\% vs 22.70\% de Pipeline A, más del doble de rendimiento
\end{itemize}

Esta superioridad se mantiene consistentemente en todas las métricas evaluadas, validando que los modelos de lenguaje grandes capturan habilidades técnicas con mayor precisión y exhaustividad que métodos sintácticos tradicionales.

\subsection{H2: Detección de Habilidades Emergentes}

La segunda hipótesis postulaba que los LLMs detectarían habilidades emergentes ausentes en taxonomías estáticas como ESCO. Los resultados confirman esta capacidad:
...
\subsection{H3: Inferencia de Habilidades Implícitas}

La tercera hipótesis planteaba que los LLMs podrían inferir habilidades implícitas de las responsabilidades descritas en ofertas laborales. Los resultados validan esta capacidad:
...
\begin{itemize}
    ...
    \item \textbf{Pipeline A optimizado} mediante 7 experimentos iterativos que mejoraron F1 de 42\% a 81\%
...
\begin{itemize}
    \item \textbf{Demostración cuantitativa} de superioridad de LLMs: +16.8\% F1 versus métodos tradicionales
...
\begin{itemize}
    ...
    \item \textbf{Pipeline A.1 descartado}: F1=48\% insuficiente para producción, aunque útil como baseline académico
...
\begin{itemize}
    \item \textbf{La iteración sistemática funciona}: Pipeline A mejoró 38 puntos porcentuales de F1 en 7 experimentos controlados
...
Finalmente, la validación cuantitativa de las tres hipótesis de investigación, la identificación honesta de limitaciones, y la documentación exhaustiva del sistema (81,000 líneas) establecen un precedente metodológico para futuros trabajos en extracción de información de ofertas laborales mediante inteligencia artificial. Este proyecto demuestra que la combinación rigurosa de métodos tradicionales de NLP, modelos de lenguaje grandes, y evaluación sistemática con \textit{gold standard} produce sistemas robustos, interpretables y de alto rendimiento aplicables a problemas reales del mercado laboral latinoamericano.
