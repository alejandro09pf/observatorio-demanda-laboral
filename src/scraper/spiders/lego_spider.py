...
class LegoSpider(BaseSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        country_urls = {
            'CO': ['https://www.lego.com/en-co/careers'],
            'MX': ['https://www.lego.com/en-mx/careers'],
            'AR': ['https://www.lego.com/en-ar/careers']
        }
        self.start_urls = country_urls.get(self.country, country_urls['CO'])

    def parse_search_results(self, response):
        """Parse search results page."""
        job_cards = response.css("div.job-card, article.career-item")
        
        for job_card in job_cards:
            job_url = job_card.css("a.job-link::attr(href), a[href*='/careers/']::attr(href)").get()
            if job_url:
