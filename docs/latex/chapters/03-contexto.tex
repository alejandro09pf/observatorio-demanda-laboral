...
\subsection{Habilidades, Competencias y Skills: Clarificación Terminológica}

En la literatura académica y profesional del mercado laboral existen tres términos frecuentemente utilizados de manera intercambiable que requieren clarificación: skill, habilidad y competencia. Aunque relacionados, presentan matices conceptuales importantes.

El término skill (del inglés) se define como la capacidad específica y demostrable para ejecutar una tarea particular, generalmente adquirida mediante entrenamiento o práctica \cite{kavargyris2025}. Es el término predominante en la literatura anglosajona y en taxonomías internacionales como ESCO y O*NET. El término habilidad es la traducción directa al español de skill y se utiliza como sinónimo en este trabajo, refiriendo a la capacidad técnica o destreza para realizar una actividad específica, como ``programar en Python'' o ``administrar bases de datos''.

El término competencia tiene un alcance más amplio en marcos como el Europeo de Cualificaciones (EQF): integra conocimientos (saber), habilidades (saber hacer) y actitudes (saber ser) para desempeñar una función profesional de manera efectiva \cite{rubio2025}. Una competencia como ``desarrollo de software'' engloba múltiples skills (Python, Git, testing) junto con conocimientos teóricos (algoritmos, arquitectura) y actitudes (resolución de problemas, colaboración).

Para este trabajo, se adopta el término skill (y su equivalente habilidad) como unidad de análisis, siguiendo la convención de la literatura de NLP aplicada al mercado laboral \cite{nguyen2024, kavargyris2025, lukauskas2023}. Operacionalmente, se define como cualquier mención textual en ofertas laborales que describa capacidades técnicas requeridas: lenguajes (``Python''), metodologías (``Scrum''), herramientas (``Docker''), conceptos técnicos (``microservicios'') o competencias transversales (``trabajo en equipo''). Los tres términos se utilizan de manera intercambiable a lo largo del documento, priorizando skill y habilidad por su especificidad y alineación con las taxonomías empleadas. Las skills operan como proxy de demanda laboral: su frecuencia refleja intensidad de demanda, y su co-ocurrencia revela ecosistemas tecnológicos \cite{lukauskas2023}.
...
Para NLP automatizado, las hard skills presentan ventaja por expresión léxica consistente (``Docker'' con variantes limitadas), mientras soft skills exhiben alta variabilidad lingüística (``trabajo en equipo'' = ``colaboración'', ``espíritu colaborativo'', ``teamwork'') \cite{campos2024}.

\subsection{Hard Skills IT en el Análisis Automatizado}

Este trabajo se enfoca exclusivamente en hard skills IT por tres razones. Primero, las nomenclaturas estandarizadas globalmente facilitan extracción automatizada con alta precisión versus la ambigüedad semántica de soft skills \cite{nguyen2024}. Segundo, la trazabilidad a taxonomías ESCO (13,939 skills) y O*NET (152 skills modernas) provee vocabularios controlados exhaustivos \cite{kavargyris2025, rubio2025}. Tercero, la identificación de tecnologías emergentes provee inteligencia accionable para actualización curricular \cite{rubio2025}.
...
Una taxonomía de habilidades es un sistema jerárquico que organiza skills en categorías, establece relaciones semánticas y provee identificadores únicos. Cumplen tres funciones principales. Primero, la estandarización mediante la unificación de variantes léxicas (``JS'' a  ``JavaScript'' en ESCO) \cite{kavargyris2025}. Segundo, la comparabilidad internacional ya que ESCO es multilingüe (27 idiomas UE), facilitando análisis transnacionales \cite{kavas2024}. Tercero, el mapeo de relaciones al conectar skills con ocupaciones y sectores económicos.

Una limitación fundamental compartida por las taxonomías es su lentitud de actualización frente al cambio tecnológico acelerado. Por ejemplo, ESCO v1.1.0 (2021) no incluye tecnologías que surgieron después de 2022, como ``ChatGPT'', ``LangChain'', ``Tailwind CSS'', ``dbt'' o ``Terraform'' \cite{rubio2025}, generando una categoría de ``skills emergentes'' sin representación en vocabularios controlados. Esta brecha temporal entre los ciclos de actualización taxonómica (cada 2-3 años) y la aparición de nuevas tecnologías (en cuestión de semanas o meses) representa un desafío metodológico significativo para los sistemas de análisis automatizado del mercado laboral tecnológico.
...
El preprocesamiento textual involucra operaciones de normalización y estructuración. Herramientas como spaCy realizan automáticamente tokenización (segmentación del texto en unidades mínimas o ``tokens'': palabras, signos de puntuación) \cite{nguyen2024} y análisis morfológico, transformando cadenas continuas en secuencias discretas procesables por algoritmos de NLP.
...
Una vez establecidos los conceptos fundamentales del dominio, es necesario analizar cómo la literatura académica y aplicada ha abordado el desafío de extraer, analizar y comprender la demanda de habilidades a partir de ofertas de empleo en línea. Si bien el objetivo es común, traducir texto en conocimiento accionable sobre el mercado laboral, las aproximaciones metodológicas varían significativamente en complejidad, escalabilidad y profundidad semántica.
...
El estudio más completo fue el de Rubio Arrubla (2025) para el mercado colombiano \cite{rubio2025}. Este trabajo construyó una base de datos masiva mediante web scraping del portal elempleo.com para el periodo 2018-2023, abarcando más de 500,000 ofertas laborales. Su principal aporte fue la caracterización cuantitativa del impacto de la pandemia, demostrando un cambio estructural: las vacantes tecnológicas aumentaron 50\% en 18 meses post-pandemia. Se observó una caída en la demanda de herramientas ofimáticas tradicionales como Excel (35.8\% en 2018 a 17.4\% en 2023) y un surgimiento exponencial de tecnologías especializadas como NoSQL (12.3\%), Django (5.5\%) y React (5.3\%) para 2023 \cite{rubio2025}.
...
Más allá de la extracción individual de habilidades, la tercera línea de investigación se centra en arquitecturas de análisis completas que estructuran datos y descubren patrones latentes de manera no supervisada, respondiendo cómo se agrupan las habilidades y evolucionan los perfiles laborales.
...
También se encontró la ausencia de validación comparativa ya que ningún estudio contrasta sistemáticamente métodos tradicionales (NER + Regex) contra LLMs en el mismo corpus, impidiendo cuantificar el valor agregado de cada enfoque. Los estudios evalúan técnicas de forma aislada sin proveer marco de validación empírica cruzada.
