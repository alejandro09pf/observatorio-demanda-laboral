...
\textit{Nota: Los valores presentados en la Tabla \ref{tab:comparacion-tecnicas-extraccion} corresponden a resultados validados sobre el gold standard de 300 ofertas laborales anotadas manualmente (100 por país: CO, MX, AR). El F1-Score Post-ESCO refleja el rendimiento después de normalización con la taxonomía ESCO. Pipeline A combina NER+Regex+ESCO en arquitectura dual. La métrica de habilidades implícitas para LLMs (126\% soft skills) indica que el modelo detectó 26\% más soft skills que las anotadas manualmente, validando la hipótesis de inferencia contextual.}

Dado que ninguna técnica individual satisface todos los requisitos, se propone una comparación empírica entre dos enfoques: \textbf{Pipeline A} (NER + Regex + ESCO) implementado para procesamiento escalable del corpus completo; y \textbf{Pipeline B} (LLMs) implementado experimentalmente sobre gold standard para enriquecimiento semántico y detección de habilidades implícitas. Esta comparación permite evaluar qué pipeline logra mejor balance entre precisión y cobertura. El Pipeline B con Gemma 3 4B fue evaluado sobre 299 de las 300 ofertas del gold standard (99.3\% cobertura), demostrando superioridad cuantitativa (F1=84.26\% vs 72.53\% Pipeline A) pero con trade-off de costo computacional 43$\times$ mayor, validando su viabilidad como método de enriquecimiento selectivo complementario al procesamiento masivo con Pipeline A \cite{li2023}.
...
\hline
Latencia (GPU) & 42.07 s/job & \textbf{15.24 s/job} \\
\hline
...
El módulo NER utilizó \texttt{es\_core\_news\_lg} con EntityRuler poblado con 666 patrones ESCO y múltiples filtros post-extracción (200+ stopwords NER, 60+ technical generic stopwords). Resultados sobre gold standard: Pipeline A (NER+Regex) alcanzó F1-Score Pre-ESCO 24.98\% y F1-Score Post-ESCO 72.53\%, con recall de 64.43\% y precision post-filtrado de 9.3\% (pre-ESCO), latencia 50-80ms. El análisis reveló que NER aportó +6.91pp de mejora Pre-ESCO respecto a regex solo, pero generó -6.64pp de degradación Post-ESCO debido a extracción de variantes textuales que no mapearon a ESCO. Conclusión: A pesar del bajo rendimiento post-normalización, NER se mantuvo activo en Pipeline A dado que aporta mejoras significativas Pre-ESCO y, considerando que la cobertura ESCO es limitada (12.6\%), se priorizó mejorar el score Pre-ESCO sobre el Post-ESCO. La arquitectura final utiliza NER+Regex con ESCO matching de dos capas (exact + fuzzy 0.85).
...
Las top 10 skills matched con ESCO fueron: Python (14 menciones), Agile (13), SQL (10), JavaScript (10), Git (8), FastAPI (8), AWS Lambda (8), Kubernetes (6), Go (6), GitLab CI/CD (6). Entre las skills sin match ESCO con frecuencia $>$3 se identificaron: Marketing (6), Salesforce (5), Notion (4), RESTful (3), revelando gaps de cobertura en herramientas SaaS modernas y conceptos de arquitectura API.

El match rate de 12.6\% es bajo, reflejando que la taxonomía ESCO v1.1.0, aunque actualizada periódicamente, presenta cobertura limitada para frameworks y tecnologías emergentes en el mercado tech latinoamericano. Sin embargo, es importante notar que no todas las habilidades no matched son genuinamente emergentes: el análisis reveló que una porción significativa corresponde a ruido de extracción (menciones incompletas, fragmentos HTML residuales, términos genéricos sin valor técnico). Para cuantificar esta problemática, se ejecutó un experimento de validación mediante fuzzy matching exhaustivo de 1,430 skills sin mapeo ESCO contra el catálogo completo, determinando que 99.6\% no tienen coincidencia razonable (score $<$ 0.85) con ninguna habilidad ESCO, confirmando que son genuinamente emergentes o ruido de extracción que requiere filtrado adicional.

Estos resultados tienen implicaciones arquitectónicas importantes para el sistema. La variación del match rate por país (CO 15.3\%, MX 11.3\%, AR 12.5\%) sugiere diferencias regionales en adopción tecnológica que deben considerarse en el análisis de clustering, potencialmente requiriendo ajuste de parámetros HDBSCAN por región. El bajo match rate valida la decisión de implementar análisis dual Pre-ESCO y Post-ESCO: mientras Post-ESCO provee normalización y comparabilidad internacional, Pre-ESCO captura la diversidad léxica completa del mercado laboral local incluyendo tecnologías de adopción reciente.
...
Se diseñó y ejecutó un experimento comparativo para evaluar los cuatro modelos LLM candidatos (Gemma 3 4B, Llama 3.2 3B, Qwen 2.5 3B, Phi-3.5 Mini). La evaluación se realizó en dos fases: primero, los cuatro modelos se compararon sobre un subconjunto de 10 ofertas laborales del gold standard para selección preliminar del modelo ganador; posteriormente, el modelo seleccionado (Gemma 3 4B) se evaluó exhaustivamente sobre las 300 ofertas completas del gold standard anotadas manualmente por expertos del dominio. El gold standard se construyó con distribución balanceada por país (100 CO, 100 MX, 100 AR) y representación diversa de roles técnicos: Backend (33.33\%), QA (16.33\%), Frontend (14.00\%), DevOps (11.67\%), Data Science (9.00\%), y otros roles especializados (15.67\%).
...
\begin{table}[H]
\centering
\caption{Comparación de Candidatos LLM - Evaluación sobre 10 ofertas}
\label{tab:llm-comparison-results}
\begin{tabular}{|p{4cm}|p{2.5cm}|p{6cm}|}
\hline
\textbf{Modelo} & \textbf{F1 Pre-ESCO} & \textbf{Observaciones Cualitativas} \\
\hline
\textbf{Gemma 3 4B} & \textbf{46.23\%} & JSON válido 99\%, sin alucinaciones sistemáticas, outputs limpios \\
\hline
Llama 3.2 3B & 39.7\% & Alucinaciones sistemáticas (skills Data Science en web dev), buen formato \\
\hline
Qwen 2.5 3B & 38.9\% & Extracciones conservadoras (10-12 skills/oferta), alta precisión/bajo recall \\
\hline
Phi-3.5 Mini & 35.2\% & Formato inconsistente (60\% JSON válido), pérdida de skills por parsing \\
\hline
\end{tabular}
\end{table}

\textit{Nota: La evaluación preliminar de los cuatro candidatos LLM se realizó sobre 10 ofertas laborales del gold standard para selección del modelo ganador. F1 Pre-ESCO corresponde al F1-Score sin mapeo taxonómico, evaluando capacidad de extracción pura. Gemma 3 4B fue seleccionado y posteriormente evaluado exhaustivamente sobre las 300 ofertas completas del gold standard, alcanzando F1 Post-ESCO de 84.26\%. Los otros tres modelos no fueron evaluados sobre el dataset completo. Todos los modelos se ejecutaron con cuantización INT4 en hardware consumer (Macbook Air M4 base).}

La evaluación comparativa determinó que \textbf{Gemma 3 4B} ofrece el mejor balance entre precisión (F1=46.23\% Pre-ESCO, 84.26\% Post-ESCO), ausencia de alucinaciones, y consistencia de formato JSON para el contexto del español latinoamericano técnico. Gemma 3 4B fue desplegado en el Pipeline B para enriquecimiento semántico de 299 ofertas del gold standard (99.3\% éxito), mientras que el Pipeline A (NER + Regex + ESCO) procesó las 300 ofertas como baseline de comparación. Los resultados de la comparación empírica demuestran trade-offs claros: Pipeline A alcanza 72.53\% F1 Post-ESCO con latencia de 1.82 s/job (ideal para procesamiento masivo), mientras que Pipeline B alcanza 84.26\% F1 Post-ESCO con 42.07 s/job (apropiado para enriquecimiento selectivo cuando precisión es crítica), ambos ejecutándose localmente sin dependencias de APIs comerciales.
...
El sistema implementa un \textbf{pipeline secuencial de 7 etapas}:
...
\caption{Arquitectura Modular del Observatorio - Pipeline de 7 Etapas}
...
El esquema constó de siete tablas principales correspondientes a las etapas del pipeline y evaluación:

\textbf{raw\_jobs}: Almacena ofertas tal como fueron scrapeadas. Campos clave: identificador UUID, portal de origen, código de país (CO/MX/AR), título, descripción, requisitos, hash SHA-256 para deduplicación exacta, y bandera de procesamiento. Incluye campos de deduplicación semántica (is\_duplicate, duplicate\_of, duplicate\_similarity\_score, duplicate\_detection\_method) para identificar ofertas similares mediante fuzzy matching.
...
\textbf{gold\_standard\_annotations}: Almacena anotaciones manuales de habilidades sobre el dataset de 300 ofertas laborales utilizadas para evaluación de pipelines. Campos principales: identificador, referencia a la oferta laboral, texto de la habilidad anotada manualmente, tipo de habilidad (hard/soft), anotador, fecha de anotación, y notas adicionales. Esta tabla permite evaluación cuantitativa de precisión mediante comparación contra ground truth.
