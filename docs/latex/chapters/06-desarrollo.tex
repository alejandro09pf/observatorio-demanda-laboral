...
El corpus se recolectó mediante scraping especializado por portal: algunos portales utilizaron únicamente \texttt{requests} + \texttt{Beautiful\-Soup4} para HTML estático (Computrabajo, ElEmpleo, InfoJobs, OCC Mundial), mientras que otros requirieron \texttt{selenium} para contenido JavaScript dinámico (Bumeran, ZonaJobs). Se implementaron estrategias anti-bloqueo: delays aleatorios 2-5s, rotación de User-Agent, y respeto de \texttt{robots.txt}. El scraping completo recolectó 56,555 ofertas brutas, procesadas durante ~72 horas distribuidas en 15 días (~150-200 ofertas/hora).
...
\begin{itemize}
    ...
    \item \textbf{Mantenibilidad}: Separación de componentes permite actualización independiente de patrones Regex sin modificar EntityRuler de NER
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{EntityRuler}: Diccionario poblado con 666 patrones de la taxonomía ESCO para reconocimiento directo de habilidades técnicas
    \item \textbf{Configuración}: Modelo pre-entrenado base sin fine-tuning. Se utilizó EntityRuler con diccionario de patrones, no se realizó entrenamiento del modelo
...
La evaluación del Pipeline A validó cuatro capacidades críticas para procesamiento masivo del corpus completo. Primero, la cobertura de skills estandarizadas alcanzó 12.6\% mediante el matcher ESCO implementado de dos capas. Segundo, la velocidad de procesamiento demostró ser 3× más rápida que NER aislado y 43× más rápida que Pipeline B, permitiendo procesamiento del corpus completo en menos de 10 horas con latencia lineal O(n). Tercero, la reproducibilidad se garantizó mediante resultados 100\% deterministas sin componentes aleatorios. Cuarto, la escalabilidad se confirmó procesando exitosamente el dataset completo de 30,660 ofertas.

Sin embargo, el Pipeline A presenta cuatro limitaciones inherentes a métodos basados en patrones. La baja precisión Pre-ESCO de 22.54\% refleja alto ruido en extracciones crudas que requiere filtrado posterior con ESCO. La cobertura ESCO limitada de 12.6\% indica que 87.4\% de skills extraídas son emergentes ausentes en la taxonomía estándar. La fragmentación léxica causa que skills compuestas como ``machine learning'' sean ocasionalmente detectadas como tokens separados. Finalmente, la sensibilidad a ruido HTML genera falsos positivos cuando las ofertas contienen marcado residual mal limpiado.
...
La decisión de implementar Pipeline B basado en LLMs se fundamentó en cuatro limitaciones críticas de Pipeline A que requerían comprensión semántica profunda. Primero, Pipeline A no detecta skills implícitas al requerir menciones explícitas en el texto, siendo incapaz de inferir ``Git'' cuando la oferta menciona ``experiencia con control de versiones''. Segundo, presenta fragmentación de skills compuestas, detectando ``machine'' y ``learning'' como tokens separados en lugar de reconocer ``Machine Learning'' como concepto único. Tercero, carece de desambiguación contextual para resolver menciones ambiguas. Cuarto, no captura sinónimos contextuales ni relaciones semánticas entre términos técnicos.

El enfoque LLM resuelve estas limitaciones mediante comprensión contextual que interpreta texto considerando la semántica completa de la oración, inferencia de skills implícitas que deduce tecnologías sin match textual directo, desambiguación semántica que utiliza el contexto de la oferta para resolver ambigüedades, normalización automática que genera skills en formato estándar, y capacidad superior para detectar habilidades emergentes ausentes en taxonomías tradicionales. La evaluación empírica demostró que Gemma 3 4B alcanza F1-Score Post-ESCO de 84.26\% comparado con 72.53\% de Pipeline A, representando una mejora de +11.73 puntos porcentuales. Adicionalmente, la arquitectura con cuantización Q4 permite inferencia en hardware consumer (4-6GB VRAM) evitando dependencia de APIs comerciales, mientras que la aplicación selectiva sobre gold standard balancea calidad versus latencia (42.07s/oferta vs. 0.97s de Pipeline A).
...
Todos los modelos se ejecutaron con cuantización INT4 mediante \texttt{bitsandbytes}, reduciendo requisitos de memoria de $\sim$16GB (FP16) a $\sim$4GB (INT4) para permitir inferencia en hardware consumer (Macbook Air M4 base). La evaluación preliminar comparativa se realizó sobre 10 ofertas laborales del gold standard, analizando tanto métricas cuantitativas (F1-Score Pre-ESCO) como comportamiento cualitativo (alucinaciones, consistencia de formato, relevancia de extracciones).
...
\begin{itemize}
    ...
    \item \textbf{Implementación}:
        \begin{itemize}
            \item Tokenización con truncamiento a 3800 tokens (límite empírico para evitar OOM en hardware consumer)
...
La evaluación exhaustiva del Pipeline B sobre 299 ofertas del gold standard validó cinco capacidades diferenciadas. La extracción de skills implícitas alcanzó cobertura de soft skills de 126\% respecto a anotación manual, confirmando inferencia contextual efectiva. La desambiguación contextual demostró alta precisión de 89.25\%, superando métodos basados en patrones. La normalización automática generó skills en formato estándar consistente facilitando mapeo ESCO. La capacidad de detectar emergent skills reveló que 59.5\% de extracciones corresponden a tecnologías modernas ausentes en ESCO v1.1.0. Finalmente, la ejecución local mediante cuantización Q4 eliminó dependencias de APIs comerciales requiriendo únicamente 4-6GB VRAM.

Sin embargo, el Pipeline B presenta cinco limitaciones operacionales. La latencia variable de 18-47× mayor que Pipeline A (mediana 17.66s vs. 0.97s) incluye outliers hasta 254 segundos para ofertas extensas. La cobertura reducida extrae 1.8× menos skills que Pipeline A (27.8 vs. 50.3 promedio). La escalabilidad limitada implica que procesar el corpus completo de 30,660 ofertas requeriría 385 horas (16 días) con tiempo promedio. El recall ligeramente inferior de 79.81\% comparado con 81.25\% de Pipeline A (-1.44pp) refleja conservadurismo en extracciones ambiguas. La dependencia de GPU con 4-6GB VRAM mínimo imposibilita ejecución en CPU puro.
