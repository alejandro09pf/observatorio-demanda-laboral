...
El esquema (Figura \ref{fig:diagrama-er}) constó de seis tablas principales correspondientes a las etapas del pipeline descritas anteriormente. Todas las tablas derivadas mantienen referencia mediante llave foránea hacia la tabla de ofertas laborales (raw\_jobs), garantizando trazabilidad completa desde cualquier resultado de análisis hasta la oferta original:
...
Las tablas de habilidades extraídas y enriquecidas mantienen referencia opcional hacia la taxonomía ESCO para efectos de normalización. La configuración detallada de PostgreSQL para procesamiento batch y las estrategias de optimización de índices se documentan en el Apéndice E.
