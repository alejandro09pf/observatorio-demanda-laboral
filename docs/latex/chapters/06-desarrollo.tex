...
El corpus se recolectó mediante scraping especializado por portal: \texttt{requests} + \texttt{Beautiful\-Soup4} para HTML estático, \texttt{selenium} para contenido JavaScript dinámico. Se implementaron estrategias anti-bloqueo: delays aleatorios 2-5s, rotación de User-Agent, y respeto de \texttt{robots.txt}. El scraping de 31,000+ ofertas tomó ~72 horas distribuidas en 15 días (~150-200 ofertas/hora).

El pipeline de limpieza aplicó cinco pasos: normalización de texto, remoción de HTML residual, detección de idioma mediante \texttt{langdetect} (español 33.2\%, inglés 52.4\%, Spanglish 14.4\%), deduplicación por \texttt{content\_hash} (424 duplicados, 1.37\%), y validación de calidad descartando ofertas incompletas (236 ofertas, 0.76\%). El resultado fue un dataset de 30,660 ofertas usables (98.87\% del total) con texto normalizado, idioma identificado, y metadata completa. La distribución temporal cubrió 7 años (2018-10-12 a 2025-10-31) con 71.23\% de ofertas fechadas, permitiendo análisis trimestral.
...
\begin{itemize}
    \item \textbf{Precisión}: Regex proporciona baseline de alta precision (86.36\% Post-ESCO) para skills estandarizadas
    \item \textbf{Cobertura}: NER incrementa recall de 40.16\% (Regex solo) a 64.43\% (combinado)
    \item \textbf{Eficiencia}: Latencia de 0.97s/oferta es 43× más rápida que Pipeline B (42.3s/oferta)
...
\begin{itemize}
    ...
    \item \textbf{ESCO match rate}: Porcentaje de skills extraídas que mapearon a taxonomía ESCO
        \begin{itemize}
            \item Pipeline A: 11.1\% (baja cobertura indica presencia de skills emergentes)
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cobertura ESCO}: 11.1\% de skills extraídas mapearon a taxonomía
\end{itemize}
...
\begin{itemize}
    \item[$\checkmark$] Alta cobertura de skills estandarizadas: 25.7\% (Regex) y 11.1\% (NER+Regex) mapearon a ESCO
...
\begin{itemize}
    \item[$\times$] Baja precision Pre-ESCO (22.54\%): Alto ruido en extracciones crudas, requiere filtrado
    \item[$\times$] Cobertura ESCO limitada (11.1\%): 89\% de skills extraídas no mapean a taxonomía estándar
...
\begin{itemize}
    \item \textbf{Precision}: LLMs alcanzan 86.36\% F1 Post-ESCO, vs. 72.53\% de Pipeline A (+13.83pp mejora)
    \item \textbf{Cobertura semántica}: Captura skills implícitas que Pipeline A omite (promedio 18.7 skills/oferta vs. 50.3, pero más relevantes)
    \item \textbf{Costo-Efectividad}: Aplicación selectiva (gold standard, muestreo temporal) balancera calidad vs. latencia
    \item \textbf{Escalabilidad}: Cuantización INT4 permite inferencia en GPUs consumer (RTX 3090), evitando dependencia de APIs comerciales
\end{itemize}
...
\begin{itemize}
    \item \textbf{Gemma 3 4B Instruct}: F1-Score 44.4\% Pre-ESCO
        \begin{itemize}
            \item Formato JSON válido en 94\% de respuestas
            \item Extracción balanceada: 15-25 skills/oferta
            \item Sin alucinaciones evidentes detectadas en revisión manual
            \item Latencia promedio: 42.3s/oferta
        \end{itemize}
...
\begin{itemize}
    \item \textbf{Mejor F1-Score}: 44.4\% superó a alternativas en +4.7pp (vs. Llama) hasta +9.2pp (vs. Phi)
    \item \textbf{Estabilidad de formato}: 94\% de respuestas en JSON válido redujo complejidad de parsing
    \item \textbf{Ausencia de alucinaciones críticas}: Revisión manual de 50 ofertas no detectó skills irrelevantes sistemáticas
    \item \textbf{Latencia aceptable}: 42.3s/oferta permitió procesamiento de 300 ofertas gold standard en $<$4 horas
\end{itemize}
