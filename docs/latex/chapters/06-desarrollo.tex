...
\begin{itemize}
    \item \textbf{Cobertura}: NER incrementa recall Post-ESCO de 73.08\% (Regex solo) a 81.25\% (Pipeline A combinado), capturando +8.17pp de skills adicionales mediante menciones contextuales
...
\begin{itemize}
    ...
    \item \textbf{Eficiencia}: Latencia de 0.97s/oferta es aproximadamente 18× más rápida que Pipeline B (mediana de 18s/oferta)
    \item \textbf{Mantenibilidad}: Separación de componentes permite actualización independiente de patrones Regex sin modificar EntityRuler (poblado con diccionario ESCO de 666 patrones)
\end{itemize}
...
\textbf{Resultados obtenidos.} La evaluación del Pipeline A sobre las 300 ofertas del gold standard produjo las siguientes métricas:
...
\begin{itemize}
    \item \textbf{Gemma 3 4B Instruct}: F1-Score 46.23\% Pre-ESCO
        \begin{itemize}
            ...
            \item Latencia: mediana 18.34s/oferta (P25-P75: 15-25s), media 42.07s (inflada por outliers)
        \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Latencia aceptable}: tiempo típico de 15-25s/oferta (mediana 18s) permitió procesamiento de 299 ofertas gold standard en 3.5 horas
\end{itemize}
