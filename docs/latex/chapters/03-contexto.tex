...
Soft Skills (habilidades transversales) son capacidades relacionales e interpersonales transversales a dominios con evaluación subjetiva y relevancia estable \cite{fareri2021}: liderazgo, comunicación, trabajo en equipo, resolución de problemas, adaptabilidad.
...
Este trabajo se enfoca exclusivamente en hard skills IT por tres razones: (1) Nomenclaturas estandarizadas globalmente facilitan extracción automatizada con alta precisión vs. ambigüedad semántica de soft skills \cite{nguyen2024}; (2) Trazabilidad a taxonomías ESCO (13k+ skills) y O*NET (233 skills subset) proveen vocabularios controlados exhaustivos \cite{kavargyris2025, rubio2025}; (3) Identificación de tecnologías emergentes provee inteligencia accionable para actualización curricular \cite{rubio2025}.
...
Una taxonomía de habilidades es un sistema jerárquico que organiza skills en categorías, establece relaciones semánticas y provee identificadores únicos. Cumplen tres funciones: (1) Estandarización: unifican variantes léxicas (``JS'' → ``JavaScript'' en ESCO) \cite{kavargyris2025}; (2) Comparabilidad internacional: ESCO es multilingüe (27 idiomas UE), facilitando análisis transnacionales \cite{kavas2024}; (3) Mapeo de relaciones: conectan skills con ocupaciones y sectores económicos.

Limitación: actualización lenta vs. cambio tecnológico. ESCO v1.1.0 (2021) excluye tecnologías post-2022: ``ChatGPT'', ``LangChain'', ``Tailwind CSS'', ``dbt'', ``Terraform'', generando ``skills emergentes'' sin representación en vocabularios controlados (análisis propio basado en inspección de ESCO v1.1.0).

El proyecto usa ESCO como taxonomía primaria \cite{kavas2024, kavargyris2025} y O*NET como complemento. Siguiendo el enfoque de estudios regionales previos que validaron el uso de O*NET como referencia para habilidades tecnológicas de alta demanda \cite{rubio2025}, se adoptó una estrategia dual PRE-ESCO (skills emergentes) + POST-ESCO (comparabilidad internacional) que balancea innovación tecnológica y rigor taxonómico.
...
El preprocesamiento es fundamental para estandarizar los datos textuales. La Tokenización consiste en segmentar el texto en unidades mínimas o ``tokens'' (generalmente palabras o signos de puntuación) \cite{nguyen2024}, transformando cadenas continuas en secuencias discretas procesables. La Lematización reduce las palabras a su forma base o raíz gramatical, permitiendo agrupar variaciones morfológicas (por ejemplo, ``programar'', ``programando'' y ``programado'' se unifican bajo el lema ``programar'') \cite{vasquezrodriguez2024}.
...
El estudio más completo fue el de Rubio Arrubla (2024) para el mercado colombiano \cite{rubio2025}. Este trabajo construyó una base de datos masiva mediante web scraping del portal elempleo.com para el periodo 2018-2023, abarcando más de 2.3 millones de ofertas laborales. Su principal aporte fue la caracterización cuantitativa del impacto de la pandemia, demostrando un cambio estructural: las vacantes tecnológicas aumentaron 50\% en 18 meses post-pandemia. Se observó una caída en la demanda de herramientas ofimáticas tradicionales como Excel (35.8\% en 2018 a 17.4\% en 2023) y un surgimiento exponencial de tecnologías especializadas como NoSQL (12.3\%), Django (5.5\%) y React (5.3\%) para 2023 \cite{rubio2025}.
...
Para México, Campos-Vázquez y Martínez Sánchez (2024) combinaron datos de encuestas oficiales con scraping, basándose en frecuencia de términos y tipología manual para segmentar habilidades \cite{campos2024}.
...
\hline
\multirow{2}{*}{LLM Llama-3 8B (CoT)} & Precisión{\fontfamily{pcr}\selectfont @}5 & 0.32 & 0.28 \\
\cline{2-4}
 & Recall{\fontfamily{pcr}\selectfont @}5 & 0.76 & 0.72 \\
\hline
Embeddings E5-large & Recall{\fontfamily{pcr}\selectfont @}10 & 0.88 & 0.92 \\
\hline
\end{tabular}
\end{table}
...
\hline
LLMs Prompting & Flexibilidad; Sin entrenamiento; Captura contexto complejo & Inconsistencia de salida; Alucinaciones; F1 bajo (36-54\% vs 59-73\% supervised) & \cite{nguyen2024} \\
\hline
