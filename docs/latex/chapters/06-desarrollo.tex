...
Con base en el diseño arquitectónico presentado en el capítulo anterior, este capítulo describe la implementación concreta de cada componente del observatorio. Se detalla la infraestructura técnica, los sistemas de extracción de habilidades, el mapeo a taxonomías estandarizadas, el sistema de clustering, y la interfaz de usuario.
...
Una vez establecida la infraestructura base, se procedió a implementar los sistemas de extracción de habilidades, componente central del observatorio. Se desarrollaron cuatro aproximaciones metodológicas para extracción automática de habilidades técnicas: Pipeline A (NER + Regex), Pipeline B (LLM), Pipeline A.1 (TF-IDF, descartado), y configuración Regex-Only para baseline.
...
Como extensión, se implementó la infraestructura base para análisis temporal futuro de evolución de clusters. El módulo permite análisis longitudinal mediante una agrupación de ofertas por trimestre, la extracción de skills por período, una generación de embeddings E5, una proyección UMAP, un clustering HDBSCAN  y por ultimo, un tracking de consistencia de clusters entre períodos consecutivos (threshold: $\geq$60\% overlap en top-20 skills). El sistema genera visualizaciones temporales (heatmaps clusters × quarters, line charts de frecuencia) para identificar patrones de adopción tecnológica.
