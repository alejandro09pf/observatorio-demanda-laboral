...
\textbf{Pipeline B (Basado en LLMs):} Empleó Large Language Models (LLMs), evaluando comparativamente Gemma 3 4B, Llama 3, Phi-3, Mistral y Qwen, para realizar extracción semántica capaz de identificar no solo habilidades explícitas sino también de inferir competencias implícitas a partir del contexto de la vacante.

Posteriormente, un módulo de mapeo normalizó las habilidades extraídas por ambos pipelines contra la taxonomía ESCO mediante dos capas: coincidencia léxica exacta y fuzzy matching con threshold de similitud 0.85. Una tercera capa experimental de búsqueda semántica usando embeddings multilingües (E5) y FAISS se deshabilitó tras detectar falsos positivos en contexto técnico. Finalmente, un módulo de análisis no supervisado aplicó una secuencia metodológica de embeddings E5 (768D), reducción de dimensionalidad con UMAP (2D) y agrupamiento density-based con HDBSCAN para identificar familias semánticas de habilidades y perfiles emergentes.

\section{Justificación de la solución}

La solución implementada se justificó como una alternativa superior y mejor adaptada para el análisis de la demanda de habilidades en América Latina, ya que abordó directamente las debilidades metodológicas identificadas en los estudios previos. A diferencia de los enfoques basados exclusivamente en reglas léxicas \parencite{aguilera2018, rubio2025}, la arquitectura de dos pipelines paralelos permitió una validación empírica cruzada: combinó la auditabilidad y alta precisión para habilidades conocidas del Pipeline A con la potencia inferencial y la capacidad de descubrir habilidades implícitas del Pipeline B.
...
Técnicamente, el sistema representó un avance en escalabilidad mediante arquitectura modular que permitió procesar el corpus completo de 30,660 ofertas con Pipeline A en ~8.3 horas (0.97s/oferta), factible para ejecución overnight. El ESCO Matcher de dos capas (exact + fuzzy) procesó ~15,000 skills únicas en ~6.5 minutos aprovechando memoización. El sistema de clustering aplicó UMAP + HDBSCAN sobre embeddings E5 (768D) generados a ~850 skills/s en GPU, completando reducción dimensional y agrupamiento de 1,698 skills en <30 segundos.
