\section*{APÉNDICE C: REQUERIMIENTOS Y ESPECIFICACIÓN FUNCIONAL DEL SISTEMA}
\addcontentsline{toc}{section}{Apéndice C: Requerimientos y Especificación Funcional}
...
\subsection*{C.1. Requerimientos del Sistema}
...
\subsubsection*{C.1.1. Requerimientos Funcionales}
...
\subsubsection*{C.1.2. Requerimientos No Funcionales}
...
\subsubsection*{C.1.3. Requerimientos de Datos}
...
\subsubsection*{C.2.1. Restricciones Técnicas}
...
\subsubsection*{C.2.2. Restricciones de Datos}
...
\subsubsection*{C.2.3. Restricciones Metodológicas}
...
\subsection*{C.3. Especificación Funcional}
...
\subsubsection*{C.3.1. Arquitectura de Pipeline de 7 Etapas}
...
\subsubsection*{C.3.2. Interfaces Críticas}

\textbf{I-1. Interfaz Scraper-Database:} Los spiders de Scrapy utilizan un pipeline personalizado (\texttt{JobPostgresPipeline}) que serializa items a formato JSON y los inserta en \texttt{raw\_jobs} con manejo de duplicados por hash SHA-256 del contenido.

\textbf{I-2. Interfaz Extractor-ESCO:} El módulo \texttt{ESCOMatcher3Layers} expone métodos: \texttt{find\_exact\_match()} y \texttt{find\_fuzzy\_match()}. El sistema implementa memoización mediante diccionario \texttt{skill\_text} $\to$ \texttt{esco\_uri} que evita remapear skills repetidas.
...
\subsubsection*{C.3.3. Casos de Uso Principales}
