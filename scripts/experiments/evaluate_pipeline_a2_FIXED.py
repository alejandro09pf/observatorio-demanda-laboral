#!/usr/bin/env python3
"""
Evaluación de Pipeline A.2 - Versión CORREGIDA
"""
...
from scripts.experiments.pipeline_a2_FIXED import (
    extract_skills_correct_approach,
    load_esco_skills_as_set,
    normalize_text
)
...
def main():
    print("=" * 70)
    print("PIPELINE A.2 - EVALUACIÓN VERSIÓN CORREGIDA")
    print("=" * 70)
    print()
    ...
    # Cargar ESCO
    print("📥 Cargando skills de ESCO (tech only)...")
    ...
    # Evaluar
    print("🚀 Evaluando Pipeline A.2 CORREGIDO...")
    print()
    ...
    for idx, (job_id, job_description) in enumerate(job_descriptions.items(), 1):
        ...
        # Extraer skills (SIN PRINT)
        import sys
        from io import StringIO
        old_stdout = sys.stdout
        sys.stdout = StringIO()  # Suprimir prints

        extracted = extract_skills_correct_approach(job_description, esco_skills_dict, max_n=4)
    ...
    # Guardar resultados
    output_path = 'outputs/evaluation/pipeline_a2_FIXED_results.json'
    ...
    print()
    print("=" * 70)
    print("📊 RESULTADOS FINALES - PIPELINE A.2 CORREGIDO")
    print("=" * 70)
    print()
    ...
    print("-" * 70)
    print(f"{'A2.FIXED (Correcto)':<30} | {overall_metrics['f1']:>6.2f}% | {overall_metrics['precision']:>6.2f}% | {overall_metrics['recall']:>6.2f}% | {overall_metrics['avg_skills_per_job']:>10.2f}")
    print(f"{'A2.2 (Tech only - anterior)':<30} | {9.73:>6.2f}% | {10.61:>6.2f}% | {8.99:>6.2f}% | {95.28:>10.2f}")
    print(f"{'A2.0 (Original - anterior)':<30} | {6.68:>6.2f}% | {5.39:>6.2f}% | {8.78:>6.2f}% | {124.57:>10.2f}")
    print()
    print("✅ ¡Evaluación completada!")
    print()
