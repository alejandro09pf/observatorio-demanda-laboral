...
Este capítulo presenta los hallazgos obtenidos tras la implementación y ejecución del observatorio. Se organiza en tres secciones principales: primero, la evaluación comparativa de los pipelines de extracción; segundo, los resultados del sistema de clustering; y tercero, el análisis del mercado laboral tecnológico latinoamericano derivado de los datos procesados.
...
Complementando la evaluación Pre-ESCO, la Tabla~\ref{tab:eval_post_esco} presenta métricas tras mapear todas las skills a taxonomía ESCO, evaluando alineación con vocabulario controlado.
...
Habiendo establecido Pipeline B (Gemma 3 4B) como el método de extracción superior, esta sección presenta los hallazgos del análisis sobre el corpus completo de 30,660 ofertas procesadas, caracterizando distribución de skills, identificando tecnologías emergentes, y documentando tendencias del mercado tech latinoamericano durante 2018-2025.
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../../outputs/clustering/final/pipeline_b_300_pre/umap_macro_centroids.png}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../../outputs/clustering/final/pipeline_b_300_post/umap_macro_centroids.png}
\end{minipage}
\caption{Vista de centroides de clusters para Pipeline B (LLM) en etapas PRE-ESCO (izquierda, 34 clusters) y POST-ESCO (derecha, 50 clusters). El tamaño de cada círculo representa la frecuencia total de menciones del cluster, revelando dominios tecnológicos de alta demanda como AWS (649 menciones PRE), Docker (87 menciones POST), TypeScript (48 POST) y Python (64 POST). El color indica pertenencia a meta-clusters jerárquicos identificados automáticamente por HDBSCAN. La etapa PRE captura diversidad léxica completa incluyendo variantes (``js'', ``postgres''), mientras POST consolida vocabulario ESCO estandarizado con mayor granularidad semántica (50 vs 34 clusters).}
\label{fig:pipeline_b_centroids}
\end{figure}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../../outputs/clustering/final/pipeline_b_300_pre/umap_scatter.png}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{../../outputs/clustering/final/pipeline_b_300_post/umap_scatter.png}
\end{minipage}
\caption{Proyección UMAP completa de Pipeline B mostrando todos los puntos individuales. (Izquierda) PRE-ESCO con 1,766 skills distribuidas en 34 clusters capturando diversidad léxica completa. (Derecha) POST-ESCO con 1,937 skills en 50 clusters tras normalización ESCO. Los clusters están etiquetados con sus tecnologías más representativas, revelando dominios tecnológicos como Backend (Docker, Kubernetes), lenguajes (TypeScript, Python), frameworks JavaScript (React, Node.js), bases de datos (SQL, PostgreSQL), infraestructura Cloud (GCP, Azure) y competencias transversales (Comunicación, Liderazgo). Los puntos grises representan ruido: 12.8\% PRE, 16.5\% POST.}
\label{fig:pipeline_b_scatter}
\end{figure}

Las visualizaciones del clustering final evidencian la capacidad del sistema UMAP+HDBSCAN de detectar estructura semántica interpretable sin supervisión. La vista de centroides PRE-ESCO identifica 34 agrupaciones tecnológicas con 1,766 skills procesadas, capturando diversidad léxica completa del corpus incluyendo variantes ortográficas y nomenclaturas no estandarizadas. La etapa POST-ESCO incrementa granularidad a 50 clusters tras mapeo a taxonomía ESCO, procesando 1,937 skills normalizadas con mejora del Silhouette Score de 0.234 a 0.348, indicando clusters más cohesivos y mejor separados. La proyección completa revela que el mercado tech latinoamericano presenta concentración en tres meta-clusters jerárquicos: tecnologías core backend-infrastructure (naranja), lenguajes y frameworks modernos (azul), y skills transversales de gestión-comunicación (gris). Los 50 clusters identificados automáticamente categorizados manualmente en 14 familias tecnológicas reflejan demanda diversificada donde Databases (916 menciones), Programming Languages (729) y DevOps (715) lideran el ecosistema.
